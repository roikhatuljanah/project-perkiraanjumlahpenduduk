...
with st.sidebar:
    ...
    if selected == "Perhitungan":
        jenis_perhitungan = st.selectbox("Jenis Perhitungan", ["Prediksi Pertumbuhan Pertahun", "Prediksi Pertumbuhan PerKelompok"])
...
def Perhitungan_page():
    ...
    if jenis_perhitungan == "Prediksi Pertumbuhan PerKelompok":
        TahunAwal = st.text_input('Tahun Awal', value='2025')
        TahunAkhir = st.text_input('Tahun Akhir', value='2029')

        if st.button("Dapatkan Prediksi Pertumbuhan"):
            try:
                # Konversi input pengguna ke tipe data yang sesuai (misalnya, float)
                tahun_awal_input = float(TahunAwal)
                tahun_akhir_input = float(TahunAkhir)

                # Pastikan model Anda memiliki metode 'predict'
                if hasattr(model, 'predict'):
                    total_prediksi = 0  # Inisialisasi total prediksi

                    for tahun in range(int(tahun_awal_input), int(tahun_akhir_input) + 1):
                        data_baru = [[tahun]]
                        prediksi_baru = model.predict(data_baru)
                        prediksi_float = float(prediksi_baru[0])
                        total_prediksi += prediksi_float  # Menambahkan prediksi ke total
                        st.write(f'Tahun {int(tahun)}: {prediksi_float:.8f}')

                    st.success(f"Total prediksi pertumbuhan penduduk dari tahun {int(tahun_awal_input)} hingga {int(tahun_akhir_input)} adalah {total_prediksi:.8f}")
                else:
    ...
    elif jenis_perhitungan == "Prediksi Pertumbuhan Pertahun":
        ...
        if st.button("Dapatkan Prediksi Pertumbuhan"):
            try:
                ...
                if hasattr(model, 'predict'):
                    ...
                    st.success(f"Prediksi pertumbuhan penduduk pada tahun {int(tahun_input)} adalah {prediksi_float:.8f}")
                else:
                    st.error("Model regresi tidak memiliki metode 'predict'. Pastikan model yang dimuat adalah model regresi yang valid.")
            except ValueError:
                st.error("Masukkan tahun yang valid. Contoh: 2025")
